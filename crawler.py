...
"""
    ...
    def save_checkpoint(self):
        ...
        try:
            ...
            if self.status_callback:
                self.status_callback('checkpoint_saved', {'path': self.checkpoint_file})
        except Exception as e:
    ...
    def load_checkpoint(self):
        if os.path.exists(self.checkpoint_file):
            try:
                ...
                logging.info(f"Checkpoint loaded: {self.checkpoint_file}")
                if self.status_callback:
                    self.status_callback('checkpoint_loaded', {'path': self.checkpoint_file})
            except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start-url", required=True, help="URL de départ pour le crawler")
    parser.add_argument("--max-depth", type=int, default=2, help="Profondeur maximale du crawl")
    parser.add_argument("--use-playwright", action="store_true", help="Utiliser Playwright pour le rendu (headless browser)")
    parser.add_argument("--download-pdf", action="store_true", help="Télécharger les fichiers PDF")
    parser.add_argument("--download-doc", action="store_true", help="Télécharger les fichiers DOC/DOCX/XLS/XLSX/PPT/PPTX")
    parser.add_argument("--download-image", action="store_true", help="Télécharger les images")
    parser.add_argument("--download-other", action="store_true", help="Télécharger les autres types de fichiers (archive, audio, vidéo)")
    parser.add_argument("--llm-provider", type=str, help="Fournisseur LLM (ex: openai)")
    parser.add_argument("--openai-api-keys", nargs='*', help="Clés API OpenAI")
    parser.add_argument("--max-tokens", type=int, default=2048, help="Nombre max de tokens pour le LLM")
    parser.add_argument("--max-urls", type=int, help="Nombre maximum d'URLs à crawler")
    parser.add_argument("--crawler-output-dir", type=str, default="./output", help="Dossier de sortie du crawler")
    parser.add_argument("--checkpoint-file", type=str, default="./checkpoint.json", help="Fichier de checkpoint")
